...
class MarketValuationDao:
    # 根据user_id搜索 work_order
    def getByUserId(self, id):
        result = MarketValuation.query.filter(MarketValuation.user_id == int(id))
        return result
    ...
    def getAll(self):
        result = MarketValuation.query.all()
        return result

    # 更新 work_order 信息
    def update(self, entity):
        result = MarketValuation.query.filter(MarketValuation.order_id == entity.order_id).first()
        result = entity
        db.session.commit()
        return result
