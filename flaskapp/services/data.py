    ...
    def get_work_order_detail_by_id(self, order_id):
        ...
        try:
            ...
            if base.user_id == session.get('id'):
                ...
                elif int(base.method) == int(COMPREHENSIVE):
                    detail = dao_service.comprehensive_valuation_dao.getByOrderId(order_id).first()
                    # tuple.append(detail)
                    tuple['detail'] = detail

                elif int(base.method) == int(EARNING):
                    detail = dao_service.earning_valuation_dao.getByOrderId(order_id).first()
                    # tuple.append(detail)
                    tuple['detail'] = detail

                elif int(base.method) == int(MARKET):
                    detail = dao_service.market_valutation_dao.getByOrderId(order_id).first()
                    # tuple.append(detail)
                    tuple['detail'] = detail
                ...
                return response_dict("工单查询成功", 200, tuple)
